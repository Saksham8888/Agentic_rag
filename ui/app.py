...
# Initialize Logfire only when a valid token is available
try:
    token = (os.getenv("LOGFIRE_TOKEN") or "").strip()
    if not token:
        print("WARNING: LOGFIRE_TOKEN is empty or None; tracing is disabled.")
...
with st.sidebar:
    st.title("🧠 Agent OS")
    st.markdown("---")
    backend_url = (os.getenv("BACKEND_URL") or "http://localhost:8000").strip() or "http://localhost:8000"
            ...
            with st.status("🔍 Agent is thinking...", expanded=True) as status:
                try:
                    ...
                    with logfire.span("📡 Calling RAG Backend"):
                        ...
                        base_url = (os.getenv("BACKEND_URL") or "http://localhost:8000").strip() or "http://localhost:8000"
